...
int check_for_event2(void);
float calculate_direction_led2(void);
void turn_to_direction2(float direction);
int current_samples3[8];
char buffer2[80];
...
int check_for_event2(void)
{
	...
	current_samples3[0]=e_get_prox(0);
	current_samples3[1]=e_get_prox(1);
	current_samples3[2]=e_get_prox(2);
	current_samples3[3]=e_get_prox(3);
	current_samples3[4]=e_get_prox(4);
	current_samples3[5]=e_get_prox(5);
	current_samples3[6]=e_get_prox(6);
	current_samples3[7]=e_get_prox(7);
    // Detect event on any of the 8 sensors
	event=0;

	for (i=0; i<8; i++) {
		if(current_samples3[i]>350) {
			event = 1;
		}
	}
...
float calculate_direction_led2(void)
{
	...
	if ( current_samples3[0]>350 )
		{direction=(PI/8.0);
		led_nb = 7;
		}
	if ( current_samples3[1]>350 )
		{direction=(2.0*PI/8.0);
		led_nb = 9;
		}
	if ( current_samples3[2]>350 )
		{direction=(4.0*PI/8.0);
		led_nb = 12;
		}
	if ( current_samples3[3]>350 )
		{direction=(6.0*PI/8.0);
		led_nb = 10;
		}
	if ( current_samples3[4]>350 )
		{direction=(8.0*PI/8.0);
		led_nb = 13;
		}
	if ( current_samples3[5]>350 )
		{direction=(10.0*PI/8.0);
		led_nb = 14;
		}
	if ( current_samples3[6]>350 )
		{direction=(12.0*PI/8.0);
		led_nb = 15;
		}
	if ( current_samples3[7]>350 )
...
void curiosity_behaviour() {
	...
	while(1) {
		...
		{
			event = check_for_event2(); // check if attacker is detected
        }      
		// if there is an event then:
		if (event==1) {
			
    	    direction = calculate_direction_led2();   // do all the calculations where the attack is coming from
        	printf("DIRECTION=%f",direction); 	
			turn_to_direction2(direction); // turn the robot to the direction the attack is coming from
			
			e_set_steps_right(0);	
			e_set_speed_left(TURN_SPEED);  // motor speed in steps/s
			e_set_speed_right(TURN_SPEED);  // motor speed in steps/s//back off	
				//slowly start speeding up?
