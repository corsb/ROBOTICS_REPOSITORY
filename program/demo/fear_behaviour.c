...
float calculate_direction_led(void)
{
	...
	int led_nb = 6;
	long int i;	
	//angle at which the sensors are at
	if ( current_samples2[0]>350 )
		{direction=(PI/8.0);	 //22.5
		led_nb = 7;
		}
	if ( current_samples2[1]>350 )//45
		{direction=(2.0*PI/8.0);
		led_nb = 9;
		}
	if ( current_samples2[2]>350 )//90
		{direction=(4.0*PI/8.0);
		led_nb = 12;
		}
	if ( current_samples2[3]>350 )//135
		{direction=(6.0*PI/8.0);
		led_nb = 10;
		}
	if ( current_samples2[4]>350 )//225
